...
export default function App() {
  ...
  const [tips, setTips] = useState({}); // { stageId: [pick1, pick2, pick3] }
  const [results, setRes] = useState({}); // { stageId: [r1, r2, r3] }
  const [search, setSearch] = useState("");
  const [isAdm, setIsAdm] = useState(false);
  const [aP, setAP] = useState(["", "", ""]);
  const [toast, setToast] = useState(null);
  const [loading, setLoading] = useState(false);
  const [allScores, setAllScores] = useState([]); // [{nickname, score}]
  const [dataReady, setDataReady] = useState(false);

  // ─── LOAD TEAMS & RIDERS FROM SUPABASE ON START ───
  useEffect(() => {
...
useEffect(() => { if (user) localStorage.setItem('rz_user', JSON.stringify(user)); }, [user]);
  ...
  const login = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        if (error) {
          if (error.code === '23505') {
            sT("Nickname bereits vergeben!");
          } else {
            sT("Fehler bei der Anmeldung");
          }
          setLoading(false);
          return;
        }
  ...
  const loadData = useCallback(async () => {
    ...
    // Load user's stage tips
    const { data: stageTips } = await supabase
      .from('stage_tips')
      .select('*')
      .eq('user_id', user.id);
    ...
    if (stageTips) {
      const tipsMap = {};
      stageTips.forEach(t => {
        tipsMap[t.stage_id] = [t.pick_1, t.pick_2, t.pick_3];
      });
      setTips(tipsMap);
    }

    // Load user's jersey tips
    ...
    if (stageResults) {
      ...
      stageResults.forEach(r => {
        resMap[r.stage_id] = [r.result_1, r.result_2, r.result_3];
      });
      setRes(resMap);
    }
  ...
  const loadLeaderboard = async () => {
    ...
    const scores = allUsers.map(u => {
      ...
      userTips.forEach(t => {
        ...
        if (res) {
          score += cSP([t.pick_1, t.pick_2, t.pick_3], res).total;
        }
      });
  ...
  const saveStageTip = async (stageId, picks) => {
    ...
    if (error) {
      sT("Fehler beim Speichern");
    } else {
      sT(`Tipp für Etappe ${stageId} gespeichert! ✅`);
      setTips(p => ({ ...p, [stageId]: picks }));
    }
  };

  // ─── SUPABASE: SAVE JERSEY TIPS ───
  const saveJerseyTips = async (jerseyTips) => {
    ...
    const { error } = await supabase
      ...
      .upsert({
        user_id: user.id,
        rosa: jerseyTips.rosa || null,
        ciclamino: jerseyTips.cicl || null,
        azzurra: jerseyTips.azz || null,
        bianca: jerseyTips.bia || null,
        updated_at: new Date().toISOString(),
      }, { onConflict: 'user_id' });
    ...
    if (error) {
      ...
      sT("Trikot-Prognosen gespeichert! 🩷");
      setJDone(true);
      setEditJ(false);
    }
  };

  // ─── SUPABASE: SAVE RESULT (ADMIN) ───
  const saveResult = async (stageId, picks) => {
    ...
    if (error) {
      ...
      sT(`✅ Ergebnis Etappe ${stageId} gespeichert`);
      setRes(p => ({ ...p, [stageId]: [...picks] }));
      // Refresh leaderboard
  ...
  const totS = useMemo(() => {
    let s = 0;
    Object.entries(results).forEach(([sid, res]) => {
      if (tips[sid]) s += cSP(tips[sid], res).total;
    });
    return s;
  }, [tips, results]);
    ...
    return (
      <div style={S.fw}>
        ...
        <div style={S.cd}>
          ...
          <div style={S.br}>
            <span style={S.bg}>🥇 10</span>
            <span style={S.bg}>🥈 7</span>
            <span style={S.bg}>🥉 5</span>
            <span style={S.bg}>🩷 25</span>
          </div>
  ...
  if (!jDone || editJ) {
    ...
    return (
      <div style={S.fw}>
        <style>{CSS}</style>
        <div style={{ ...S.cd, maxWidth: 500, textAlign: "left" }}>
              ...
              <div key={j.id} style={{ marginBottom: 14 }}>
                ...
                {sl && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 5 }}>
                    <Badge id={sl.i} sz={26} />
                    <span style={{ fontSize: 12, color: "#999" }}>{sl.n} · {tm?.n}</span>
                  </div>
                )}
              </div>
            );
          })}
          <button style={{ ...S.bp, marginTop: 18, opacity: af ? 1 : .4 }} onClick={() => af && saveJerseyTips(jTips)}>
            PROGNOSEN SPEICHERN →
          </button>
          <button style={S.sk} onClick={() => { setJDone(true); setEditJ(false); }}>
            {editJ ? "Abbrechen" : "Später ausfüllen"}
          </button>
        </div>
        {toast && <div style={S.tt}>{toast}</div>}
      </div>
  ...
  const tabItems = [
    { id: "tippen", l: "🎯 Tippen" },
    { id: "auswertung", l: "📊 Auswertung" },
    { id: "rangliste", l: "🏆 Rangliste" },
    { id: "tipps", l: "📋 Meine Tipps" },
    ...(isAdm ? [{ id: "admin", l: "⚙️ Admin" }] : []),
  ];
    ...
    <div style={S.aw}>
      ...
      <header style={S.hd}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={S.sp}>
            <span style={{ color: "#888", fontSize: 10 }}>PUNKTE</span>
            <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: "#f472b6" }}>{totS}</span>
          </div>
          <span style={S.ub}>{user.nickname}</span>
          {isAdm && <span style={S.ab}>ADMIN</span>}
        </div>
      </header>

      <nav style={S.tb}>
        {tabItems.map(t => (
          <button key={t.id} style={tab === t.id ? { ...S.ta, ...S.to } : S.ta}
      ...
      <main style={S.mn}>
           ...
           {!JER.every(j => jTips[j.id]) && !jerseyLocked && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {ST.map(s => {
                ...
                const c = isDL(s);
                return (
                  <button key={s.id} style={{ ...S.sc2, borderColor: hr ? "#4ade80" : ht ? "#f472b6" : c ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.07)", opacity: c && !ht && !hr ? .4 : 1 }} onClick={() => setSel(s)}>
                    <div style={{ ...S.sn, background: hr ? "rgba(74,222,128,0.1)" : ht ? "rgba(244,114,182,0.1)" : "rgba(255,255,255,0.03)" }}>
                      <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 17, color: hr ? "#4ade80" : ht ? "#f472b6" : "#555" }}>{s.id}</span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, color: "#555" }}>{fD(s.d)} · {s.km} km · Start {s.tm}</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#ddd", marginTop: 1 }}>{s.t}</div>
                      <div style={{ fontSize: 11, color: "#666", marginTop: 1 }}>{tE(s.y)} {tL(s.y)}</div>
                    </div>
                    <span style={{ fontSize: 15 }}>{hr ? "✅" : ht ? "📝" : c ? "🔒" : "○"}</span>
                  </button>
                );
              })}
            </div>
        ...
        {tab === "tippen" && sel && (
          <div>
            ...
            <div style={{ textAlign: "center", marginBottom: 18 }}>
              ...
              <h2 style={{ ...S.lg, fontSize: 24, marginTop: 3 }}>{sel.t}</h2>
            ...
            <div style={{ display: "flex", gap: 7, marginBottom: 14, flexWrap: "wrap" }}>
              {pickLabels.map((l, i) => {
                ...
                return (
                  <div key={i} style={{ flex: "1 1 180px", padding: "11px 13px", borderRadius: 10, border: `1px solid ${r ? "#f472b6" : "rgba(255,255,255,0.08)"}`, background: r ? "rgba(244,114,182,0.05)" : "rgba(255,255,255,0.015)" }}>
                    ...
                    {r ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                        ...
                        {!cl && !hR && (
                          <button onClick={() => unR(i)} style={{ background: "rgba(255,255,255,0.06)", border: "none", color: "#777", borderRadius: 6, width: 24, height: 24, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
                        )}
                      </div>
                    ) : (
            ...
            {hR && (
              <div style={{ background: "rgba(74,222,128,0.06)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: 11, padding: 18, marginBottom: 18, textAlign: "center" }}>
                  ...
                  {results[sel.id].map((rid, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <Badge id={rid} sz={26} />
                        <span style={{ fontSize: 13, color: "#ccc" }}>{["🥇", "🥈", "🥉"][i]} {r?.n}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(230px,1fr))", gap: 4 }}>
                  {fR.map(r => {
                    ...
                    return (
                      <button key={r.i} style={{ ...S.rc, opacity: isSel ? .3 : 1, borderColor: isSel ? "#f472b6" : "rgba(255,255,255,0.05)" }} onClick={() => !isSel && nE !== -1 && pickR(r.i, nE)}>
                        <Badge id={r.i} sz={32} />
                        <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {tab === "auswertung" && (
          <div>
            <h2 style={S.ti}>ETAPPEN-AUSWERTUNG</h2>
            ...
            <div style={{ ...S.sc, marginTop: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button style={S.bs} onClick={() => setEditJ(true)}>✏️ Bearbeiten</button>
              </div>
              {JER.map(j => {
        ...
        {tab === "rangliste" && (
          <div>
            <h2 style={S.ti}>RANGLISTE</h2>
            <p style={S.su}>{eS.length} von 21 Etappen ausgewertet · {lb.length} Teilnehmer</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {lb.map((u, idx) => (
                <div key={idx} style={{ ...S.lr, background: u.isMe ? "rgba(244,114,182,0.08)" : "rgba(255,255,255,0.015)", borderColor: u.isMe ? "#f472b6" : "rgba(255,255,255,0.05)" }}>
                  ...
                  <div style={{ flex: 1, fontWeight: u.isMe ? 700 : 400, color: u.isMe ? "#fff" : "#ccc", fontSize: 14 }}>
                    {u.nickname || u.n} {u.isMe && <span style={{ color: "#f472b6" }}>(DU)</span>}
                  </div>
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color: "#f472b6" }}>{u.score || u.s || 0}</div>
                </div>
              ))}
            </div>
            <div style={{ ...S.sc, marginTop: 24 }}>
            ...
            {Object.keys(tips).length === 0 && !JER.some(j => jTips[j.id]) ? (
              <div style={S.em}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center", marginTop: 14 }}>
                  ...
                  <button style={S.bs} onClick={() => setEditJ(true)}>🩷 Trikot-Prognosen</button>
                </div>
              </div>
            ) : (
              <>
                <div style={{ ...S.sc, marginBottom: 18 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    ...
                    <button style={S.bs} onClick={() => setEditJ(true)}>
        ...
        {tab === "admin" && isAdm && (
          <div>
            <h2 style={S.ti}>⚙️ ERGEBNISSE EINTRAGEN</h2>
            <p style={S.su}>Etappe wählen, Top-3 eintragen.</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 18 }}>
              {ST.map(s => (
                <button key={s.id} onClick={() => { setSel(s); setAP(results[s.id] || ["", "", ""]); }}
                  style={{ width: 40, height: 40, borderRadius: 7, border: "1px solid rgba(255,255,255,0.07)", cursor: "pointer", fontFamily: "'Bebas Neue',sans-serif", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", background: sel?.id === s.id ? "#f472b6" : results[s.id] ? "rgba(74,222,128,0.12)" : "rgba(255,255,255,0.03)", color: sel?.id === s.id ? "#000" : results[s.id] ? "#4ade80" : "#555" }}>
                  {s.id}
                </button>
              ))}
            </div>
            {sel && (
              <div style={S.sc}>
                ...
                <button style={{ ...S.bp, marginTop: 6, opacity: aP.every(Boolean) ? 1 : .4 }}
                  onClick={() => saveResult(sel.id, aP)}>
                  ERGEBNIS SPEICHERN
                </button>
              </div>
            )}
      ...
      <footer style={{ textAlign: "center", padding: 16, fontSize: 10, color: "#222", letterSpacing: 2, fontFamily: "'Bebas Neue',sans-serif", borderTop: "1px solid rgba(255,255,255,0.03)" }}>
...
const S = {
  ...
  ds: { color: "#777", fontSize: 13, lineHeight: 1.7 },
  ...
  hn: { color: "#3a3a3a", fontSize: 11, marginTop: 12 },
  ...
  ta: { background: "none", border: "1px solid transparent", borderRadius: 7, padding: "6px 12px", color: "#555", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", whiteSpace: "nowrap" },
  to: { background: "rgba(244,114,182,0.08)", borderColor: "rgba(244,114,182,0.35)", color: "#f472b6" },
